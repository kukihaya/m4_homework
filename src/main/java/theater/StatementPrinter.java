...
public class StatementPrinter {
    ...
    private int getVolumeCredits(Performance performace) {
        int result = 0;
        result += Math.max(performace.getAudience() - Constants.BASE_VOLUME_CREDIT_THRESHOLD, 0);
        // add extra credit for every five comedy attendees
        if ("comedy".equals(getPlay(performace).getType())) {
            result += performace.getAudience() / Constants.COMEDY_EXTRA_VOLUME_FACTOR;
        }
        return result;
    }
