...
public class StatementPrinter {
    ...
    public String statement() {
        int totalAmount = 0;
        int volumeCredits = 0;
        final int loanLimit = 40000;
        final int standardFee = 1000;
        final int daysPerPeriod = 30;
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            switch (play.getType()) {
                case "tragedy":
                    thisAmount = loanLimit;
                    if (p.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                        thisAmount += standardFee * (p.getAudience() - daysPerPeriod);
                    }
                    break;
                case "comedy":
            ...
            result.append(String.format(
                    "  %s: %s (%s seats)%n",
                    play.getName(),
                    frmt.format(thisAmount / centsPerDollar),
                    p.getAudience()
            ));
            totalAmount += thisAmount;
        }
        result.append(String.format("Amount owed is %s%n", frmt.format(totalAmount / centsPerDollar)));
